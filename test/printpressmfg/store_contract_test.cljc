(ns printpressmfg.store-contract-test
  "The Store contract as executable tests. Single MemStore backend --
  see `printpressmfg.store` ns docstring for why a second (Datomic-
...
(deftest batch-upsert-merges-preserving-untouched-fields
  (let [s (seeded)]
    (store/commit-record! s {:effect :batch/upsert :path ["batch-001"]
                             :value {:product-type :flexographic-press}})
    (is (= :flexographic-press (:product-type (store/batch s "batch-001"))))
...
(deftest shipment-propose-commits-and-advances-sequence-and-batch-quantity
  (let [s (seeded)]
    ...
    (is (= 18.0 (:shipped-units (store/batch s "batch-001")))
        "8.0 seeded + 10.0 committed")))

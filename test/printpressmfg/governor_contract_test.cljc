(ns printpressmfg.governor-contract-test
  ...
  Press Systems) machinery-safety conformity mark') implemented
  faithfully. The single invariant under test:

    PrintPressAdvisor never schedules maintenance, flags a safety
    concern, or coordinates a shipment the Special-Purpose Machinery
  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :log-production-batch :effect :propose :subject "batch-001"
                   :patch {:product-type :offset-press}} coordinator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= :offset-press (:product-type (store/batch db "batch-001"))) "SSoT actually updated")
...
(deftest invalid-registration-accuracy-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t10b" {:op :log-production-batch :effect :propose :subject "batch-001"
                                   :patch {:registration-accuracy-mm 999.0}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-registration-accuracy} (-> (store/ledger db) last :basis)))
    (is (not= 999.0 (:registration-accuracy-mm (store/batch db "batch-001"))) "fabricated registration-accuracy-mm never lands in the SSoT")))
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t12" {:op :flag-safety-concern :effect :propose :subject "concern-1"
                                    :value {:equipment-id "fab-001" :severity :moderate
                                            :description "impression-cylinder nip-point pressure anomaly"}}
                       coordinator)]
      (is (= :interrupted (:status res)))

(ns printpressmfg.scene-render-test
  "OPT-IN 3D-visualization E2E gate (not part of `clojure -M:test`'s
  default \"test\" dir -- see deps.edn's `:visualize` alias). Verifies
  TWO things about `printpressmfg.scene/unit->render-ir`:

    1. Structural conformance to `kami.webgpu.ir`'s v1 render-IR
       contract, checked with the REAL `kami.webgpu.ir/valid?`
       validator rather than a hand-rolled shape check (the hand-rolled
       one lives in `test/printpressmfg/scene_test.cljc`, which must
       stay runnable without a renderer).
    2. The scene actually DRAWS: baked with the REAL
       `kami.webgpu.geometry` mesh generators and rendered in a REAL
       headless WebGL2 Chromium via `kami.playwright/eval-page`, then
       pixel-verified with `readPixels`.

  Methodology is taken unchanged from the sibling isic-2813 repo's own
  `test-visualize/pressureequip/scene_test.cljc` (ADR-2800000100),
  which in turn mirrors kotoba-lang/webgpu's `playwright_mesh_test.clj`
  -- textbook perspective/lookAt/translate matrices, GLSL mesh shaders,
  drawElements, readPixels, and per-instance projected-centre sampling
  rather than frame-wide colour histogramming. Nothing new is invented
  here; it is extended from 5 instances to this scene's 11.

  NOTE on what the per-instance pixel assertion does and does not
  prove: it samples the pixel each instance's own origin projects to
  and requires it to differ from a known-empty background corner. For
  an instance that is OCCLUDED by another (this scene stacks parts
  along one optical axis, so e.g. the wafer sits under the immersion
  hood), the sampled pixel legitimately belongs to the occluder. The
  assertion is therefore 'geometry was drawn at this screen location',
  not 'this specific instance is unoccluded' -- which is exactly what a
  depth-tested render of a stacked assembly should be claimed to show."
...
(defn- bake-mesh
  ...
  `kami.webgpu.ir/instance-size` convention; for `:cylinder` the
  footprint diameter lives in `w` (== `d`)."
  [{:keys [geo size]}]
  (let [[w h d] size]
    (case geo
...
(defn- the-unit []
  (facts/unit-type-by-id :unit/arf-immersion-lithography-scanner))

(deftest unit-render-ir-conforms-to-v1-shape
  (let [scene-ir (scene/unit->render-ir (the-unit))]
    (is (ir/valid? scene-ir)
        "printpressmfg.scene/unit->render-ir output conforms to kami.webgpu.ir's v1 render-IR shape (checked with the real ir/valid?)")
    (is (= 11 (count (:instances scene-ir)))
        "frame + stage + wafer + immersion hood + optics column + reticle stage + illuminator + purge plenum + laser source + 2 load ports")
    (is (= #{:box :cylinder} (set (map :geo (:instances scene-ir))))
        "only the two parametric primitive kinds this scene uses")))

(deftest scanner-renders-in-real-headless-webgl2-browser
  (let [scene-ir (scene/unit->render-ir (the-unit))
        instances (:instances scene-ir)
        {:keys [eye target]} (:globals scene-ir)
        meshes (mapv bake-mesh instances)
        width 800 height 600
        fov 55 near 0.1 far 60.0
        js (str
            ...
            "  return {geo: inst.geo, xy: [sx,sy], inFrame: (sx>=0&&sx<W&&sy>=0&&sy<H), rgba: px(sx,sy)};"
            "});"
    ...
    (doseq [{:keys [geo xy rgba inFrame]} (:samples r)]
      (println "   " geo "@" xy (if inFrame "" "(OUT OF FRAME)") "->" rgba))
    ...
    (is (= 11 (count (:samples r))))
    (doseq [{:keys [geo xy inFrame]} (:samples r)]
      (is inFrame
          (str "the scene's own camera must frame every instance; a " geo
               " projected to " xy " which is outside the viewport")))
    (doseq [{:keys [geo rgba]} (:samples r)]
      (is (far-enough? rgba (:corner r))
          (str "geometry is drawn at the " geo " instance's projected centre"
               " (differs from the empty background corner), rgba=" rgba)))))

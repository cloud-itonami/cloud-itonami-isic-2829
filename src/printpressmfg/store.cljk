(ns printpressmfg.store
  "SSoT for the other-special-purpose-machinery (ISIC 2829) plant-
  operations coordination actor, illustrated with industrial printing-
  press manufacturing (offset, flexographic, gravure, digital-inkjet,
  screen-printing and die-cutting presses), behind a `Store` protocol
  so the backend is a swap, not a rewrite -- the same seam every
  `cloud-itonami-isic-*` actor in this fleet uses.

  Scope note: like its siblings (`cloud-itonami-isic-2823`'s own
  `metalmachmfg.store`, `cloud-itonami-isic-2819`'s own
  ...
  is self-contained (no external printing-press-manufacturing
  ...
  Four kinds of entity live here:
    - `batches`          -- the central entity. A production batch's
                             product-type/registration-accuracy/
    ...
    - `equipment`         -- a fabrication-line/assembly-commissioning-
    ...
    - `maintenance`       -- a scheduled fabrication/assembly-
                             commissioning-bench-equipment maintenance-
                             window DRAFT against a piece of equipment
                             (`printpressmfg.registry`'s
    ...
    - `shipments`         -- a proposed outbound printing-press-
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (printpressmfg.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (printpressmfg.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :product-type :offset-press
                :model "OFF-6C-1020" :registration-accuracy-mm 0.05
                :quantity-units 25.0 :defect-rate-percent 0.6
                :verified? true :registered? true
                :shipped-units 8.0
                :last-assessed "2026-06-01"}
   "batch-002" {:id "batch-002" :product-type :flexographic-press
                :model "FLX-Wide-880" :registration-accuracy-mm 0.08
                :quantity-units 15.0 :defect-rate-percent 0.4
                :verified? true :registered? true
                :shipped-units 14.0
                :last-assessed "2026-06-01"}
   "batch-003" {:id "batch-003" :product-type :gravure-press
                :model "GRV-Cyl-450" :registration-accuracy-mm 0.12
                :quantity-units 10.0 :defect-rate-percent 1.0
...
(defn- sample-equipment []
  {"fab-001" {:id "fab-001" :kind :fabrication-line
   ...
   "bench-002" {:id "bench-002" :kind :press-commissioning-bench
                :verified? false :registered? false
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `printpressmfg.registry` and returns
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `printpressmfg.registry` and
...
(defn sample-data!
  ...
  UNVERIFIED/unregistered press-commissioning-bench unit (blocks any

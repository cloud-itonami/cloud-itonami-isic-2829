(ns printpressmfg.operation
  "PrintPressOperationActor -- one plant-operations coordination
  request = one supervised actor run, expressed as a langgraph-clj
  StateGraph. The advisor (PrintPressAdvisor) is sealed into a single
  node (:advise); its proposal is ALWAYS routed through the
  Special-Purpose Machinery Plant Operations Governor (:govern) and
...
(defn build
  "Compiles a PrintPressOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `printpressmfg.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; PrintPressAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Special-Purpose Machinery Plant Operations Governor -- independent censor.
      (g/add-node :govern

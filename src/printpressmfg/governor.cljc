(ns printpressmfg.governor
  "Special-Purpose Machinery Plant Operations Governor -- the
  independent compliance layer that earns the PrintPressAdvisor the
  ...
  2006/42/EC) or ANSI B65.1 (Safety Standard for Printing Press
  Systems) machinery-safety conformity mark (an authority this actor
  ...
  `:special-purpose-machinery-plant-operations-governor` (see
  ...
  gate (SOFT -- asks a human to look, and the human may approve):
    ...
    3. Closed effect allowlist     -- is the PROPOSAL's own `:effect`
                                       ...
                                       actuate` or `:assembly-
                                       commissioning-bench/run`) is the
                                       'direct fabrication/assembly-
    ...
    5. Certification-authority
       blocked                     -- ANY proposal (any op) whose own
                                       ...
                                       2006/42/EC) or ANSI B65.1
                                       (Safety Standard for Printing
                                       Press Systems) machinery-safety
                                       conformity mark -- an authority
                                       exclusively reserved to the
                                       accredited machinery-safety
   ...
   12. Invalid defect-rate         -- for `:log-production-batch`, if
                                       ...
                                       reading (`printpressmfg.registry/
   ...
   13. Confidence floor / high-
       stakes gate                  -- LLM confidence below threshold,
                                       ...
                                       human may approve."
  (:require [printpressmfg.registry :as registry]
...
(defn- certification-authority-blocked-violations
  ...
  to self-issue a CE (EU Machinery Directive 2006/42/EC) or ANSI B65.1
  (Safety Standard for Printing Press Systems) machinery-safety
  conformity mark -- an authority exclusively reserved to the
  accredited machinery-safety certification body, never this actor. No
  ...
  (let [payload (or (:value proposal) (:patch proposal))]
    (when (true? (:issue-certification? payload))
      [{:rule :certification-authority-blocked
        :detail "機械安全適合性(CE 2006/42/EC・ANSI B65.1等)の自己発行提案は恒久的に禁止 -- 認証機関の専権事項"}])))
...
(defn- invalid-registration-accuracy-violations
  "For `:log-production-batch`, if the patch declares a
  `:registration-accuracy-mm` that is not a physically plausible
  proof-run color-registration reading, reject rather than let
  fabricated/sensor-error data through."
  [{:keys [op]} proposal]
  (when (= op :log-production-batch)
    (let [mm (:registration-accuracy-mm (:value proposal))]
      (when (and (some? mm) (not (registry/registration-accuracy-mm-valid? mm)))
        [{:rule :invalid-registration-accuracy
          :detail (str mm "mm は物理的に妥当な見当合わせ精度試験値の範囲外")}]))))

(defn- invalid-defect-rate-violations
  "For `:log-production-batch`, if the patch declares a
  `:defect-rate-percent` that is not a physically plausible reading,
  reject rather than let a fabricated/sensor-error reading through."
  [{:keys [op]} proposal]
  (when (= op :log-production-batch)
...
(defn check
  "Censors a PrintPressAdvisor proposal against the governor rules.
  ...
  (let [hard (into []
                   (concat (no-propose-effect-violations request)
                           ...
                           (invalid-registration-accuracy-violations request proposal)

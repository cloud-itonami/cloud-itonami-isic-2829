(ns printpressmfg.advisor
  "PrintPressAdvisor -- the *contained intelligence node* for the
  other-special-purpose-machinery (ISIC 2829) plant-operations
  coordination actor, illustrated here with industrial printing-press
  manufacturing (offset presses, flexographic presses, gravure
  presses, digital-inkjet presses, screen-printing presses, die-cutting
  presses).

  It normalizes production-batch patches (product-type/registration-
  accuracy-mm/quantity/defect-rate), drafts a fabrication/assembly-
  commissioning-bench-equipment maintenance scheduling proposal
  against a piece of equipment, drafts a safety-concern flag, and
  drafts an outbound printing-press-machinery shipment coordination
  ...
  fabrication/assembly-line-equipment actuation, freight dispatch, or
  machinery-safety certification -- see README `What this actor does
  NOT do`. Every output is censored downstream by
  `printpressmfg.governor` before anything touches the SSoT.
  ...
  Proposal shape (all kinds):
    {:summary    str            ; human-facing draft / finding
     :rationale  str            ; why -- informational only, NOT trusted
                                 ...
                                 ; check (see `printpressmfg.governor`)
  ...
  `printpressmfg.governor` HARD-holds any request that doesn't, so a
  mis-wired caller can never reach a commit path even if this advisor
  were compromised."
  (:require #?(:clj  [clojure.edn :as edn]
               :cljs [cljs.reader :as edn])
            [clojure.string :as str]
            [printpressmfg.registry :as registry]
...
(defn- log-production-batch
  ...
  registration-accuracy, quantity, defect-rate or verification status.
...
(defn- schedule-maintenance
  "Draft a fabrication/assembly-commissioning-bench-equipment
  ...
  registered?) in its rationale, but `printpressmfg.governor` NEVER
...
(defn- flag-safety-concern
  ...
  `printpressmfg.phase`: no phase ever adds this op to a phase's
  `:auto` set; `printpressmfg.governor` also always escalates on
...
(defn- coordinate-shipment
  "Draft an outbound printing-press-machinery shipment coordination
  ...
  `printpressmfg.governor` NEVER trusts it: it independently
  recomputes whether the batch's own cumulative-shipped quantity plus
  this claim would exceed the batch's own recorded quantity before
...
(def ^:private system-prompt
  (str "あなたは特殊用途機械(印刷機-オフセット機・フレキソ機・グラビア機・"
       "デジタルインクジェット機・スクリーン印刷機・打ち抜き機等)製造工場"
       ...
       "機械安全適合性(CE 2006/42/EC・ANSI B65.1等)を自己発行する提案を"
...
(defn- parse-proposal
  ...
  yields a safe low-confidence noop so `printpressmfg.governor`
...
(defn trace
  ...
  [request proposal]
  {:t          :printpressmfg-advisor-proposal
   :op         (:op request)

(ns printpressmfg.registry
  "Pure-function domain logic for the other-special-purpose-machinery
  (ISIC 2829) plant-operations coordination actor -- equipment/batch
  verification, shipment-quantity recompute, product-type validation,
  registration-accuracy plausibility validation, defect-rate
  ...
  pre-existing `kotoba-lang/printpressmfg`-style capability library to
  wrap (verified: no such repo exists). The domain logic therefore
  lives here as pure functions, re-verified INDEPENDENTLY by
  `printpressmfg.governor` -- the same 'ground truth, not self-report'
  discipline every sibling actor's own registry establishes (e.g.
  `metalmachmfg.registry/shipment-quantity-exceeded?` from
  `cloud-itonami-isic-2823`, and `weighpkgmfg.registry` from
  ...
  assembly-commissioning-bench-line equipment or dispatching a real
  freight carrier, and never the act of issuing a CE (EU Machinery
  Directive 2006/42/EC) or ANSI B65.1 (Safety Standard for Printing
  Press Systems) machinery-safety conformity mark (this actor NEVER
  does any of those -- see README `What this actor does NOT do`).

  SCOPE: ISIC 2829 (Manufacture of other special-purpose machinery) is
  a RESIDUAL n.e.c. category (printing machinery, plastics/rubber-
  working machinery, mining/construction-adjacent specialty machines).
  This build picks ONE concrete illustrative product line: industrial
  printing-press manufacturing -- fabrication and assembly/
  commissioning-bench lines producing finished offset, flexographic,
  gravure, digital-inkjet, screen-printing and die-cutting presses.
  ...
  fabrication/assembly-line equipment directly, and it never stands in
  for the machinery-safety certification authority that issues CE (EU
  Machinery Directive 2006/42/EC) or ANSI B65.1 machinery-safety
  conformity marks.")
...
(def valid-product-types
  ...
  through."
  #{:offset-press :flexographic-press :gravure-press
    :digital-inkjet-press :screen-printing-press :die-cutting-press})

(def registration-accuracy-mm-min
  "Physical floor for a batch's own proof-run color-registration
  accuracy reading (misalignment measured in millimetres during
  fabrication/assembly-commissioning-bench proof-run testing), never
  negative."
  0.0)

(def registration-accuracy-mm-max
  "Physical ceiling for a batch's own proof-run color-registration
  accuracy reading, in millimetres. Informed by real industrial
  printing-press commissioning practice: production-quality color
  registration is typically held well under 0.1mm, and even a badly
  out-of-tolerance press being flagged for rework rarely reports
  misalignment beyond a few millimetres -- a reading above this
  ceiling is implausible sensor/QC data, not a real proof-run
  registration reading on any standardized class of printing-press
  machinery."
  5.0)

(def defect-rate-min-percent
  "Physical floor for a batch's own fabrication/assembly-commissioning
  defect-rate reading (zero defects is the best possible outcome,
  never negative)."
  0.0)

(def defect-rate-max-percent
  "Physical ceiling for a batch's own fabrication/assembly-
  commissioning defect-rate reading -- a batch cannot reject more than
  100% of its own output. A reading above this is implausible sensor/
  QC data, not a real batch."
  100.0)
...
(defn batch-verified?
  ...
  (i.e. its product-type/registration-accuracy/quantity/defect-rate
...
(defn registration-accuracy-mm-valid?
  "Is `mm` a physically plausible proof-run color-registration
  accuracy reading (misalignment in millimetres, measured during
  fabrication/assembly-commissioning-bench proof-run testing)?
  Rejects nil, non-numbers, negative values, and values beyond
  `registration-accuracy-mm-max` -- a fabricated or sensor-error
  reading, never let through as a real test-result fact."
  [mm]
  (and (number? mm)
       (>= (double mm) registration-accuracy-mm-min)
       (<= (double mm) registration-accuracy-mm-max)))

(defn defect-rate-valid?
  "Is `percent` a physically plausible batch fabrication/assembly-
  commissioning defect-rate reading? Rejects nil, non-numbers,
...
(defn- unsigned-certificate
  ...
  B65.1 (Safety Standard for Printing Press Systems) machinery-safety
...
(defn register-maintenance
  ...
  fabrication/assembly-commissioning-bench-equipment maintenance
  window against a verified, registered piece of equipment. Pure
  function -- does not actuate the fabrication/assembly-line equipment
  or execute any maintenance; it builds the RECORD a plant coordinator
  would keep. `printpressmfg.governor` independently re-verifies the
...
(defn register-shipment
  ...
  outbound printing-press-machinery shipment against a verified,
  ...
  keep. `printpressmfg.governor` independently re-verifies the
